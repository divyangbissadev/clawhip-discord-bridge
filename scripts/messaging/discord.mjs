...
export class DiscordChannelAdapter extends BaseMessagingAdapter {
  constructor({ token, channelId, fetchImpl = fetch }) {
    super({ provider: "discord", fetchImpl });
    this.token = token;
  ...
  extractMessageText(message) {
    return extractDiscordMessageText(message);
  }
  ...
  async fetchReactionUsers(messageId, emoji, limit = 25) {
    ...
    const response = await this.request(
      `/channels/${this.channelId}/messages/${messageId}/reactions/${encodedEmoji}?limit=${limit}`,
      { method: "GET" }
    );
    return response.json();
  }
